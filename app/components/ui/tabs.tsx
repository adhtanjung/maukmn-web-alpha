...
export interface TabsProps
	...
	/** Unique ID for the motion layoutId - required when using multiple Tabs on the same page */
	layoutId?: string;
}

interface TabsContextValue extends VariantProps<typeof tabsVariants> {
	layoutId?: string;
}
...
const Tabs = React.forwardRef<
	...
	TabsProps
>(({ className, variant = "default", layoutId, ...props }, ref) => (
	<TabsContext.Provider value={{ variant, layoutId }}>
		<TabsPrimitive.Root
			ref={ref}
...
export interface TabsTriggerProps
	...
	/** Custom class for the active indicator background */
...
>(({ className, activeClassName, children, ...props }, ref) => {
	const { variant, layoutId } = React.useContext(TabsContext);
	...
	// Check if this trigger is active by observing data-state attribute
	React.useEffect(() => {
	...
	// For variants that don't use motion, render normally
	if (variant !== "rounded" && variant !== "pill") {
		return (
	...
	// For rounded and pill variants, use motion animation
	return (
		<TabsPrimitive.Trigger
			ref={triggerRef}
			className={cn(
